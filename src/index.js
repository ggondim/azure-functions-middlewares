...
async function processPipeline(pipeline, context, catchFunction) {
  ...
  for (let i = 0; i < pipeline.length; i++) {
    ...
    if (shouldRun) {
      const result = await middleware.func(context, STOP_SIGNAL);
      if (result) {
